...
try:
    from petri_logging import log_event
except ImportError:
    # Если модуль логирования недоступен, создаем заглушку
...
class PetriNetWidget(QWidget):
    ...
    def set_layout_mode(self, mode: str):
        ...
        # Разрешаем все режимы раскладки
        valid_modes = ("rows", "hier_demo", "fsm", "hierarchical", "orthogonal", "organic")
    ...
    def _compute_layout(self):
        ...
        # Всегда возвращаем размеры элементов
        return place_radius, trans_width, trans_height
    ...
    def _normalize_positions(self, pos_dict, place_radius, trans_width, trans_height):
        ...
        log_event(f"=== _normalize_positions: получено {len(pos_dict) if pos_dict else 0} позиций ===")
        
        if not pos_dict:
            log_event("pos_dict пустой, используем fallback на rows layout")
            # Если нет позиций, используем fallback на rows layout
            self._compute_layout_rows(place_radius, trans_width, trans_height)
            return
        ...
        if not all_x or not all_y:
            # Если нет координат, используем fallback
            self._compute_layout_rows(place_radius, trans_width, trans_height)
            return
        ...
        scale = min(scale_x, scale_y) * 0.85  # Более консервативный масштаб
        
        # НЕ ограничиваем масштаб сохраненным значением для новых алгоритмов
        # (только для FSM используем сохраненный масштаб)
        if self.layout_mode != "fsm":
            self._last_scale = None
        
        scale = max(0.1, min(scale, 3.0))  # Увеличен диапазон для больших графов
        ...
        # Заполняем недостающие места и переходы
        widget_center_x = width / 2
        widget_center_y = height / 2
        
        for i in range(self.model.P):
            if self.place_positions[i] == QPointF(0, 0):
                # Размещаем недостающие места по кругу
                angle = 2 * math.pi * i / self.model.P
                x = widget_center_x + 100 * math.cos(angle)
                y = widget_center_y + 100 * math.sin(angle)
                self.place_positions[i] = QPointF(x, y)
        
        while len(self.transition_positions) < self.model.T:
            # Размещаем недостающие переходы по кругу
            t_idx = len(self.transition_positions)
            angle = 2 * math.pi * t_idx / self.model.T
            x = widget_center_x + 150 * math.cos(angle)
            y = widget_center_y + 150 * math.sin(angle)
            self.transition_positions.append(QPointF(x, y))
    ...
    def _compute_layout_hierarchical(self, place_radius, trans_width, trans_height):
        ...
        """
        log_event("=== Начало _compute_layout_hierarchical ===")
        
        if not HAS_NETWORKX:
            log_event("HAS_NETWORKX = False, используем fallback на rows layout")
            self._compute_layout_rows(place_radius, trans_width, trans_height)
            return
        
        log_event(f"HAS_NETWORKX = True, HAS_PYDOT = {HAS_PYDOT}")
        
        G = self._model_to_networkx_graph()
        if G is None:
            log_event("_model_to_networkx_graph вернул None, используем fallback на rows layout")
            self._compute_layout_rows(place_radius, trans_width, trans_height)
            return
        
        log_event(f"Граф создан: {len(G.nodes())} узлов, {len(G.edges())} ребер")
        
        try:
            # Пробуем использовать Graphviz через pydot
            if HAS_PYDOT:
                log_event("Пробуем использовать Graphviz dot через pydot")
                ...
                try:
                    ...
                    log_event(f"Graphviz dot успешно: получено {len(pos)} позиций")
                except Exception as e:
                    log_event(f"Ошибка Graphviz dot: {str(e)}")
                    raise
            else:
                log_event("HAS_PYDOT = False, используем встроенный иерархический алгоритм NetworkX")
                # Fallback: используем иерархический алгоритм NetworkX
                # Создаем уровни на основе структуры графа
                try:
                    # Используем multipartite_layout для иерархического размещения
                    # Сначала определяем уровни узлов
                    levels = {}
                    ...
                    # Если нет начальных узлов, все на уровне 0
                    if not levels:
                        for node in G.nodes():
                            levels[node] = 0
                    else:
                        # Распространяем уровни через BFS
                        queue = [node for node in levels.keys()]
                        while queue:
                            current = queue.pop(0)
                    ...
                    # Используем multipartite_layout если доступен
                    if hasattr(nx, 'multipartite_layout'):
                        pos = nx.multipartite_layout(G, subset_key='level', align='vertical')
                    else:
                        # Иначе создаем иерархическое размещение вручную
                        pos = {}
                        num_levels = len(level_groups)
                        width = 1000
                        height = 800
                        for level, nodes in level_groups.items():
                            y = height * (level + 1) / (num_levels + 1)
                            num_nodes = len(nodes)
                            for i, node in enumerate(nodes):
                                x = width * (i + 1) / (num_nodes + 1)
                                pos[node] = (x, y)
                except Exception as e2:
                    log_event(f"Ошибка при создании иерархического layout: {str(e2)}")
                    # Если не получилось, используем простой иерархический layout
                    pos = {}
                    nodes = list(G.nodes())
                    num_nodes = len(nodes)
                    width = 1000
                    ...
                    for i, node in enumerate(nodes):
                        ...
                        y = height / 2
                        pos[node] = (x, y)
                    log_event(f"Использован простой иерархический layout: {len(pos)} позиций")
        except Exception as e:
            log_event(f"Критическая ошибка в hierarchical layout: {str(e)}")
            # Если все не удалось, используем простой иерархический layout
            pos = {}
            nodes = list(G.nodes())
            num_nodes = len(nodes)
            width = 1000
            ...
            for i, node in enumerate(nodes):
                ...
                y = height / 2
                pos[node] = (x, y)
            log_event(f"Использован простой fallback layout: {len(pos)} позиций")
        
        # Пост-обработка: Grid Snap для Y координат (округление до уровней)
        if pos:
            log_event(f"Пост-обработка: {len(pos)} позиций перед нормализацией")
            ...
            log_event(f"Найдено {len(y_values)} уровней Y")
            ...
            log_event("ОШИБКА: pos пустой после вычисления!")
            return
        
        log_event("Вызываем _normalize_positions для hierarchical")
    ...
    def _compute_layout_orthogonal(self, place_radius, trans_width, trans_height):
        ...
        """
        log_event("=== Начало _compute_layout_orthogonal ===")
        ...
        try:
            if HAS_PYDOT:
                log_event("Пробуем использовать Graphviz dot с splines='ortho'")
                ...
                log_event("HAS_PYDOT = False, используем spring_layout fallback")
                pos = nx.spring_layout(G, k=2.0, iterations=50)
                log_event(f"Spring layout: получено {len(pos)} позиций")
        except Exception as e:
            log_event(f"Ошибка в orthogonal layout: {str(e)}, используем spring_layout fallback")
            pos = nx.spring_layout(G, k=2.0, iterations=50)
            log_event(f"Spring layout fallback: получено {len(pos)} позиций")
        
        log_event("Вызываем _normalize_positions для orthogonal")
        self._normalize_positions(pos, place_radius, trans_width, trans_height)
        # Исправляем перекрытия
        self._fix_place_overlaps(place_radius)
        self._fix_all_overlaps(place_radius, trans_width, trans_height)
    ...
    def _compute_layout_organic(self, place_radius, trans_width, trans_height):
        ...
        """
        log_event("=== Начало _compute_layout_organic ===")
        ...
        try:
            ...
            if HAS_PYDOT:
                log_event("Пробуем использовать Graphviz neato через pydot")
                try:
                    pos = nx.nx_pydot.graphviz_layout(G, prog='neato')
                    log_event(f"Graphviz neato успешно: получено {len(pos)} позиций")
                except Exception as e:
                    log_event(f"Ошибка Graphviz neato: {str(e)}")
                    pos = None
            else:
                log_event("HAS_PYDOT = False, пропускаем Graphviz neato")
                pos = None
            
            # Если Graphviz не доступен или не сработал, используем force-directed алгоритмы
            if pos is None or not pos:
                log_event("Graphviz недоступен или не сработал, используем force-directed алгоритмы")
                # Пробуем Kamada-Kawai (встроенный в NetworkX) - дает органический вид
                if len(G.nodes()) >= 3:
                    log_event("Пробуем Kamada-Kawai layout")
                    try:
                        pos = nx.kamada_kawai_layout(G)
                        log_event(f"Kamada-Kawai успешно: получено {len(pos)} позиций")
                    except Exception as e:
                        log_event(f"Ошибка Kamada-Kawai: {str(e)}, используем spring_layout")
                        # Если Kamada-Kawai не работает, используем spring с большим k для органического вида
                        pos = nx.spring_layout(G, k=3.0, iterations=200, seed=42)
                        log_event(f"Spring layout: получено {len(pos)} позиций")
                else:
                    log_event(f"Граф слишком мал ({len(G.nodes())} узлов), используем spring_layout")
                    # Для малых графов используем spring с большим k
                    pos = nx.spring_layout(G, k=3.0, iterations=200, seed=42)
                    log_event(f"Spring layout: получено {len(pos)} позиций")
        except Exception as e:
            log_event(f"Критическая ошибка в organic layout: {str(e)}")
            # Fallback на spring layout с параметрами для органического вида
            pos = nx.spring_layout(G, k=3.0, iterations=200, seed=42)
            log_event(f"Spring layout fallback: получено {len(pos)} позиций")
        ...
        if pos:
            ...
            log_event(f"Применен scale_factor = {scale_factor}")
        else:
            log_event("ОШИБКА: pos пустой после вычисления!")
            return
        
        log_event("Вызываем _normalize_positions для organic")
        self._normalize_positions(pos, place_radius, trans_width, trans_height)
        # Исправляем перекрытия
        self._fix_place_overlaps(place_radius)
        self._fix_all_overlaps(place_radius, trans_width, trans_height)
    ...
    def paintEvent(self, event):
        ...
        try:
            ...
            # Гарантируем, что painter всегда закрывается
            painter.end()
